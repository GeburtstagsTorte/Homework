...
class Button:
    ...
    def __init__(self, pos, width, height, color=(120, 120, 120), text='', text_size=15, text_color=(0, 0, 0),
        ...
        if self.visible:
            ...
            if self.mod is None:
                ...
                if self.border is not None:
                    ...
                    pygame.draw.circle(surface, self.border, [pos[0], pos[1] + height // 2], height//2 + 1)
                    pygame.draw.circle(surface, self.border, [pos[0]+width, pos[1]+height//2], height//2 + 1)

                pygame.draw.circle(surface, color, [pos[0], pos[1] + height // 2], height // 2)
                pygame.draw.circle(surface, color, [pos[0] + width, pos[1] + height // 2], height // 2)
                pygame.draw.rect(surface, color, (pos[0], pos[1], width, height))
...
def main():
    ...
    game_exit = False
    mouse_click = False
    button = Button((50, 50), 180, 60, (0, 0, 0), 'button', text_size=15, text_color=(255, 255, 255))
    while not game_exit:
