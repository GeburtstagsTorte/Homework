    ...
    def shift_size(text, size, font, max_length, dec_by=1):
        ...
        if dec_by > 0:
            if txt_dimension[2] > max_length:
                return Box.shift_size(text, size - dec_by, font, max_length, dec_by)
        if dec_by < 0:
...
class TextBox:
    ...
    """
    basic Textbox with 2 basic designs:
        accessing design with mod=0 or 1
            0: basic rectangle
            1: rectangle with semi circles

    get input with Textbox.input

    improvements to make:
        - deleting written things smoothly
    ...
    def render(self):
        if self.visible:
            ...
            if self.button:
                self.enter_button.render(self.surface)
...
class CenterBox:
    ...
    """
    useful function to center rectangles
    """
...
class Text:
    ...
    def shift_size(text, size, font, max_length, dec_by=1):
        """
        different version of the other function. Should work for different tasks
        """
        fnt = pygame.font.SysFont(font, size)
        txt = fnt.render(text, True, (0, 0, 0))
        ...
        if txt_length > max_length:
            return Text.shift_size(text, size - dec_by, font, max_length, dec_by)
        return size

    @staticmethod
    def render_text(font, size, color, information, aa=True):
        label = []
        fnt = pygame.font.SysFont(font, size)

        for text in information:
            label.append(fnt.render(text, aa, color))
...
def main():
    ...
    game_exit = False
    mouse_click = False
    textbox_input = TextBox(screen, (50, 50), 200, 40, typeface='Arial', button=False, button_length=120, mod=0)
    # x = 110
    # y = 70
    # frame_pos = 10, 10
    ...
    while not game_exit:
        ...
        # pygame.draw.rect(screen, (255, 0, 0), (CenterBox.scale_pos((15, 20), frame_pos, x, y, frame_x, frame_y)[0],
        # CenterBox.scale_pos((15, 20), frame_pos, x, y, frame_x, frame_y)[1], 110, 70))
