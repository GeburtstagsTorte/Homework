...
class Button:
    ...
    def __init__(self, surface, pos, width, height, color=(120, 120, 120),
                 text='', text_size=10, text_color=(0, 0, 0),
                 font='Courier New', image=None, image2=None, mod=None,
                 border=None, extend=False):
        ...
        if self.visible:
            ...
            if self.collide():
                ...
                if self.extend:
                    if self.image is not None and self.image2 is None:
                        ...
                        image = pygame.transform.scale(self.image2,
                                (int(self.width * 1.1), int(self.height * 1.1)))

                    pos = [self.pos[0] - int(self.width * 0.025),
                           self.pos[1] - int(self.height * 0.025)]
            ...
            if self.mod is None:
                ...
                if self.border is not None:
                    ...
                    pygame.draw.circle(self.surface, self.border,
                                       [pos[0], pos[1] + height // 2], height//2 + 1)
                    pygame.draw.circle(self.surface, self.border,
                                       [pos[0]+width, pos[1]+height//2], height//2 + 1)

                pygame.draw.circle(self.surface, color,
                                   [pos[0], pos[1] + height // 2], height // 2)
                pygame.draw.circle(self.surface, color,
                                   [pos[0] + width, pos[1] + height // 2], height // 2)
                pygame.draw.rect(self.surface, color, (pos[0], pos[1], width, height))
            ...
            txt_rect = txt.get_rect(center=(pos[0] + 0.5 * width,
                                            pos[1] + 0.5 * height))
    ...
    def collide(self):
        ...
        if self.mod == 1 or self.mod is None:
            ...
            if self.pos[0] <= x <= self.pos[0] + self.width and \
                    ...
                    or (self.height // 2)**2 >= \
                                            (self.pos[0] - x) ** 2 + \
                                            (self.pos[1] - y + self.height // 2) ** 2 \
                    or (self.height // 2)**2 >= (self.pos[0] + self.width-x)**2 + \
                                    (self.pos[1]-y + self.height // 2)**2:
                return True
        return False
